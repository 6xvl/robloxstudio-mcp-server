/**
 * Luau that drives Studio's device simulator and network simulation.
 *
 * Ported from Chrrxs/robloxstudio-mcp (MIT). Neither of these needs a plugin endpoint:
 * StudioDeviceSimulatorService and NetworkSettings are reachable from any Luau the plugin
 * runs, so the tools generate a script and send it down the execute-luau route that
 * already exists. Generated rather than hand-written per call because the payload is
 * JSON-encoded into the source, which keeps quoting and escaping out of it entirely.
 */


// Roblox caps simulated packet loss at 0.5 percent; a higher number is silently ignored,
// so it is refused here instead of looking applied.
