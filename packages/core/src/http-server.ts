...
interface StreamableHttpConfig {
  ...
  robloxHook?: RobloxOfficialMCPClient | null;
  activeStudioMap?: Map<string, string>;
}
...
export const TOOL_HANDLERS: Record<string, ToolHandler> = {
  ...
  // ===== Extended toolset (6xvl plugin v1.1+) =====
  get_handler_health: (tools) => tools.getHandlerHealth(),

  terrain_fill_block: (tools, body) => tools.terrainFillBlock(body),
  terrain_fill_ball: (tools, body) => tools.terrainFillBall(body),
  terrain_fill_wedge: (tools, body) => tools.terrainFillWedge(body),
  terrain_fill_cylinder: (tools, body) => tools.terrainFillCylinder(body),
  terrain_clear: (tools) => tools.terrainClear(),
  terrain_get_materials: (tools) => tools.terrainGetMaterials(),
  terrain_read_voxels: (tools, body) => tools.terrainReadVoxels(body),

  lighting_list_presets: (tools) => tools.lightingListPresets(),
  lighting_set_preset: (tools, body) => tools.lightingSetPreset(body),
  lighting_get: (tools) => tools.lightingGet(),
  lighting_set: (tools, body) => tools.lightingSet(body),

  diff_subtree: (tools, body) => tools.diffSubtree(body),

  animation_play: (tools, body) => tools.animationPlay(body),
  animation_stop: (tools, body) => tools.animationStop(body),
  animation_list_tracks: (tools, body) => tools.animationListTracks(body),
  animation_load_keyframes: (tools, body) => tools.animationLoadKeyframes(body),

  particle_create: (tools, body) => tools.particleCreate(body),
  particle_tune: (tools, body) => tools.particleTune(body),

  sound_play_preview: (tools, body) => tools.soundPlayPreview(body),
  sound_stop_preview: (tools, body) => tools.soundStopPreview(body),
  sound_load_from_assetid: (tools, body) => tools.soundLoadFromAssetId(body),
  sound_set_eq: (tools, body) => tools.soundSetEq(body),
  sound_set_reverb: (tools, body) => tools.soundSetReverb(body),

  tween_preview: (tools, body) => tools.tweenPreview(body),

  textchat_configure: (tools, body) => tools.textchatConfigure(body),
  textchat_create_command: (tools, body) => tools.textchatCreateCommand(body),
  textchat_list_channels: (tools) => tools.textchatListChannels(),

  material_list_variants: (tools) => tools.materialListVariants(),
  material_create_variant: (tools, body) => tools.materialCreateVariant(body),
  material_override_base: (tools, body) => tools.materialOverrideBase(body),

  profile_start: (tools, body) => tools.profileStart(body),
  profile_stop: (tools) => tools.profileStop(),
  profile_snapshot: (tools) => tools.profileSnapshot(),

  humanoid_description_apply: (tools, body) => tools.humanoidDescriptionApply(body),
  attribute_schema_validate: (tools, body) => tools.attributeSchemaValidate(body),
  streamingenabled_audit: (tools) => tools.streamingenabledAudit(),
  reparent_instance: (tools, body) => tools.reparentInstance(body),
  insert_service: (tools, body) => tools.insertService(body),
  generate_mesh: (tools) => tools.generateMesh(),
  generate_material: (tools) => tools.generateMaterial(),
  generate_procedural_model: (tools) => tools.generateProceduralModel(),
  rojo_status: (tools) => tools.rojoStatus(),
  ...
  solo_playtest: (tools, body) => tools.soloPlaytest(body.action, body.mode, body.timeout),
  multiplayer_playtest: (tools, body) => tools.multiplayerPlaytest(body.action, body.numPlayers, body.target, body.testArgs, body.value, body.timeout),
  set_device_simulator: (tools, body) => tools.setDeviceSimulator(body),
  get_device_simulator_state: (tools, body) => tools.getDeviceSimulatorState(body),
  capture_device_matrix: (tools, body) => tools.captureDeviceMatrix(body),
  set_network_profile: (tools, body) => tools.setNetworkProfile(body),
  get_simulation_state: (tools, body) => tools.getSimulationState(body),
  reset_simulation_state: (tools, body) => tools.resetSimulationState(body),
  capture_script_profiler: (tools, body) => tools.captureScriptProfiler(body),
  capture_micro_profiler: (tools, body) => tools.captureMicroProfiler(body),
  ...
  generate_model: (tools, body) => tools.generateModel(body),
...
export function createHttpServer(tools: RobloxStudioTools, bridge: BridgeService, allowedTools?: Set<string>, serverConfig?: StreamableHttpConfig) {
  ...
  app.get('/health', (req, res) => {
    ...
    res.json({
      ...
      livenessNonce: bridge.getLivenessNonce(),
    });
  });
  ...
  app.post('/response', (req, res) => {
    ...
    // A Studio successfully delivered a response — bump the liveness nonce so
    // proxies watching /health can distinguish a live primary from a stale one.
    bridge.bumpLivenessNonce();
    ...
    app.post('/mcp', async (req, res) => {
      try {
        ...
        server.setRequestHandler(ListToolsRequestSchema, async () => {
          const ours = filteredTools.map(t => ({
            name: t.name,
          ...
          const routingTools = [
            {
              name: 'list_studios',
              description: 'List connected Roblox Studio plugin instances.',
              inputSchema: { type: 'object', properties: {} },
            },
            {
              name: 'set_active_studio',
              description: 'Set active Studio target for subsequent tool calls (role or instanceId).',
              inputSchema: {
                type: 'object',
          ...
          const hookTools = serverConfig.robloxHook?.isConnected() ? serverConfig.robloxHook.getTools() : [];
        ...
        server.setRequestHandler(CallToolRequestSchema, async (request) => {
          ...
          // Roblox official MCP forwarding
          const robloxName = RobloxOfficialMCPClient.stripPrefix(name);
          if (robloxName && serverConfig.robloxHook?.isConnected()) {
            try {
              return await serverConfig.robloxHook.callTool(robloxName, args ?? {});
            } catch (error) {
          ...
          // Built-in routing
          // Per-MCP-session active studio: HTTP transport may serve multiple Claude clients
          // concurrently on the same port. Key the active-studio map by the Mcp-Session-Id
          // header so clients don't clobber each other. Fallback to '_default' when absent.
          const sessionId = (req.headers['mcp-session-id'] as string) || '_default';
          if (name === 'list_studios') {
            return { content: [{ type: 'text', text: JSON.stringify({
              studios: bridge.getInstances(),
              active: serverConfig.activeStudioMap?.get(sessionId) || null,
            }) }] };
          }
