...
import { resolveStudioExe as resolveStudioExeShared } from './studio-exe.js';
import {
...
export function resolveStudioExe(): string {
  // Delegates to studio-exe.ts. The copy that lived here searched LOCALAPPDATA only, which
  // is the per-user install -- measured on a machine-wide install, the running Studio was
  // under Program Files (x86) and this threw. Kept exported so existing callers still work.
  return resolveStudioExeShared();
}

async function resolveStudioExeAsync(): Promise<string> {
  // Same resolution as the sync form. It was async only to keep the WSL cmd.exe hop off the
  // event loop, and that hop is memoised on first use, so there is nothing left to await.
  return resolveStudioExeShared();
}
